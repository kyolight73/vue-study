    ...
    <div class="py-4 container-fluid">
      <div class="row">
        ...
        <div class="col-md-4">
          ...
          <div>
    <input type="text" v-model="searchId" placeholder="Nhập title" @input="filterData">
    
    <p>Kết quả tìm kiếm:</p>
    <ul>
      <li v-for="item in searchResult" :key="item.id">
        ID: {{ item.title }} - Index: {{ item.id }}
      </li>
    </ul>
  </div>
        </div>
...
export default {
  ...
  data() {
    return {
      showMenu: false,
      jsonData: [],
      searchResult: null, // Biến để lưu trữ kết quả tìm kiếm
      searchId: "", // Biến để lưu trữ giá trị tìm kiếm
    };
  },
  methods: {
    filterData() {
      this.searchResult = this.jsonData.filter(item => item.title.includes(this.searchId));
    }
  },
  components: { TestCard, ArgonInput, ArgonButton },

  mounted() {
     axios.get('http://jsonplaceholder.typicode.com/posts') // Thay thế URL với URL của API thực tế
      .then(response => {
        this.jsonData = response.data; // Cập nhật dữ liệu JSON từ API
        this.searchResult = this.jsonData; // Hiển thị toàn bộ dữ liệu JSON ban đầu
      })
      .catch(error => {
